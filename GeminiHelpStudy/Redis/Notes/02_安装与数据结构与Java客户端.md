...
> [!note] OS/DB 补充 —— SDS（Simple Dynamic String）源码分析
> Redis 没有使用 C 语言的原生 `char*`，而是自实现了 **SDS（简单动态字符串）**：
>
> ```
> SDS 内存结构（sdshdr8 为例）:
>
> ┌──────────┬──────────┬──────────┬──────────────────────┐
> │  len (1B) │ alloc (1B)│ flags (1B)│  buf[] (N字节)        │
> │  已用长度  │  总分配量  │  头部类型  │  实际字符数据 + '\0'   │
> └──────────┴──────────┴──────────┴──────────────────────┘
>
> SDS 有 5 种头部类型（sdshdr5/8/16/32/64），根据字符串长度自动选择
> ```
>
> 相比 C 原生字符串，SDS 的三大优势：
> 1. **O(1) 获取长度**：直接读 `len` 字段，C 字符串需要 `O(N)` 遍历到 `\0`
> 2. **二进制安全**：SDS 靠 `len` 判断结束，不依赖 `\0`，可以存储任意二进制数据（图片、序列化对象等）
> 3. **空间预分配 + 惰性释放**：
>    - 扩容时分配额外空间（类似 Java `ArrayList` 的扩容策略），减少频繁 `realloc` 调用
>    - 缩容时不立即释放内存（惰性释放），避免频繁申请/归还内存
>
> 这与 CMU 15-213 中讲的 **动态内存分配（malloc/free）** 最佳实践完全一致：减少内存碎片，批量分配。
>
> **三种编码的选择逻辑**：
> ```
> int    → 存纯整数，直接用 long 存储，零内存开销（不需要 SDS 头部）
> embstr → 字符串 ≤ 44 字节，RedisObject 头 + SDS 头 + 数据 一次 malloc 分配
>           总大小 ≤ 64 字节（一个 CPU Cache Line），对 CPU 缓存极友好！
> raw    → 字符串 > 44 字节，RedisObject 和 SDS 分两次 malloc
> ```
> CPU Cache Line 通常是 64 字节，embstr 把整个对象控制在一个 Cache Line 内，
> 访问时一次加载即可，减少 cache miss。这是典型的 **数据局部性（Locality）** 优化。

> [!question] 深度思考 —— 为什么 embstr 的阈值是 44 字节而不是其他值？
>
> **A：这是 Redis 精心计算的结果**。
>
> Redis 对象头（redisObject）固定 16 字节：
> - 第1~4字节：4 bit type（类型）+ 4 bit encoding（编码）+ 24 bit LRU（时钟）= 32 bit = 4 字节
> - 第5~8字节：refcount（引用计数）= 4 字节
> - 第9~16字节：\*ptr（数据指针）= 8 字节
>
> sdshdr8 头部固定 3 字节（len + alloc + flags）。
> 加上 1 字节的 `\0` 终止符，以及 16 + 3 + 1 = 20 字节的固定开销。
>
> 64（Cache Line）- 20（固定开销）= **44 字节**。
>
> 这就是为什么 44 字节是分界线——超过它就无法装进一个 Cache Line 了。
...
> [!note] OS/DB 补充 —— Hash 的底层编码转换
> Hash 有两种底层编码，Redis 会根据数据量**自动切换**：
>
> ```
> Hash 编码转换示意:
>
...
> ┌────────────────────────────────────────────────────────┐
> │                  ZipList（压缩列表）                      │
> │  [prevlen][encoding][len][data][prevlen][encoding]...  │
> │  一段连续内存，字段和值交替存放                             │
> │  优点：内存紧凑，CPU 缓存命中率高                          │
> │  缺点：修改操作需要移动大段内存（O(N)）                     │
> └────────────────────────────────────────────────────────┘
>          ↓ 当 字段数 > 128 或 任意值长度 > 64 字节时，触发转换
> ┌────────────────────────────────────────────────────────┐
> │                  HashTable（哈希表）                      │
> │  数组 + 链表（链地址法解决冲突）                            │
> │  优点：O(1) 增删查改                                      │
...
> ```
>
> **渐进式 rehash**：当 HashTable 需要扩容/缩容时，Redis 不会一次性迁移所有数据
> （如果 Hash 里有 100 万字段，一次迁移会阻塞主线程很久）。
> 而是维护两个哈希表（ht[0] 和 ht[1]），每次有读写操作时**顺带迁移少量数据**，
> 直到 ht[0] 为空，再把 ht[1] 改名为 ht[0]。
>
> 类比：搬家时不是一次搬完，而是每天往新家搬一箱东西，直到搬完。
>
> 这和 Java `HashMap` 的 `resize()` 是**一次性**迁移的方式不同——Redis 牺牲少量空间换取服务不中断。
>
> **配置调整**（`redis.conf`）：
> ```bash
> hash-max-ziplist-entries 128  # ZipList 最大字段数（默认 128）
> hash-max-ziplist-value   64   # ZipList 最大值长度（默认 64 字节）
> ```
> 如果你存的对象字段不超过 128 个且值都很短，ZipList 能**节省 60% 以上内存**，非常值得保持。

> [!question] 深度思考 —— ZipList 是如何做到省内存的？
>
> **A：ZipList 把多个节点紧密排列在连续内存中，消除了指针开销。**
>
> 普通双向链表每个节点需要 2 个指针（前驱 + 后继），在 64 位系统上各占 8 字节，
> 一个节点的指针开销 = 16 字节，但数据本身可能只有几字节。
>
> ZipList 用**前缀长度（prevlen）** 替代前驱指针，用**编码（encoding）** 替代类型指针，
> 每个节点的元数据只有 2~10 字节，大幅节省内存。
>
> 缺点：连锁更新（Cascade Update）——修改中间某个节点的长度，可能导致后续所有节点的 prevlen 字段都需要更新（O(N)）。
> Redis 7.0 引入 **ListPack** 彻底解决了这个问题（移除了 prevlen，改用尾部长度字段）。
...
> [!note] DB/OS 补充 —— QuickList 底层结构（Redis 3.2+）
> List 的底层编码是 **QuickList**，它是 ZipList 和双向链表的结合体：
>
> ```
> QuickList 结构示意:
>
...
>  ┌──────┐
>  │ head │──→ ┌──────────────┐  ←prev  ┌──────────────┐  ←prev  ┌──────────────┐
>  │ tail │    │  quicklistNode│ ──────→ │  quicklistNode│ ──────→ │  quicklistNode│
>  │ ...  │    │ ┌──────────┐ │  next   │ ┌──────────┐ │  next   │ ┌──────────┐ │
...
>              └──────────────┘         └──────────────┘         └──────────────┘
>
>  每个 QuickListNode 内部是一个 ZipList，默认最多存 128 个元素
>  ZipList 元素满了就新建一个 QuickListNode
> ```
>
> **设计精妙之处**：
> - 纯 ZipList：数据量大时，单块连续内存太大，内存碎片严重，且修改操作慢（需要 realloc）
> - 纯双向链表：每个节点独立分配，指针开销大，内存不连续，缓存命中率低
> - **QuickList = 分段的 ZipList**：每段大小合理，兼顾了内存效率和修改性能
>
> 类比：QuickList 就像图书馆的**书架分区**——整个馆的书按学科分成若干书架（QuickListNode），每个书架上紧密排列（ZipList）。既不是把所有书堆在一起，也不是每本书单独放一个房间。
>
> **配置**（`redis.conf`）：
> ```bash
> list-max-ziplist-size 128   # 每个 QuickListNode 最多元素数
> list-compress-depth   0     # 两端节点压缩深度（0=不压缩，内部节点可用 LZF 压缩）
> ```
...
> [!note] DB 补充 —— Set 的底层编码（IntSet vs HashTable）
> Set 也有两种底层编码：
>
> ```
> Set 编码转换:
...
> │  IntSet（整数集合）                           │
> │                                             │
> │  encoding: INT_ENC_INT16/32/64              │
> │  length: 3                                  │
> │  contents: [100][200][300]  ← 有序数组！      │
> │                                             │
> │  查找用二分搜索，O(log N)                     │
> │  内存极紧凑（每个元素 2/4/8 字节，无指针开销）  │
> └─────────────────────────────────────────────┘
>
> 任意元素是字符串 或 元素数量 > 512
>     ↓
...
> ```
>
> IntSet 有个有趣的**自动升级**机制：
> - 若集合里都是 16 位整数，用 `INT_ENC_INT16`（每个 2 字节）
> - 插入一个 32 位整数 → 整个 IntSet 升级为 `INT_ENC_INT32`（每个扩为 4 字节）
> - 升级时需要重新分配内存并迁移所有数据，这是 O(N) 操作
>
> 这与 C++ `std::vector<int16_t>` 升级到 `std::vector<int32_t>` 的语义类似，是典型的**空间-时间权衡**。

> [!question] 深度思考 —— Set 做集合运算时底层用了什么算法？
>
> **A：取决于集合大小和编码方式，有多种策略。**
>
> - `SINTER`（交集）：遍历**最小集合**的每个元素，检查其是否在其他集合中存在（O(N×M)，N 最小集合大小，M 其他集合数量）
> - `SUNION`（并集）：逐一合并，HashTable 保证去重，O(N) 总元素数
> - `SDIFF`（差集）：从第一个集合出发，过滤掉在其他集合中出现的元素，O(N)
>
> 生产中用 `SINTERSTORE` 把结果存入新 key，避免一次性返回超大结果集阻塞客户端。
...
> [!note] OS/DB 补充 —— 跳表（SkipList）图解
>
> ```
> 跳表结构（存储 1, 3, 7, 12, 20, 35 ）:
>
> Level 3:  head ──────────────────────────────→ 35 → NULL
> Level 2:  head ─────────────→  12 ──────────→ 35 → NULL
> Level 1:  head ────→  7 ────→  12 → 20 ─────→ 35 → NULL
> Level 0:  head → 1 → 3 → 7 → 12 → 20 → 35 → NULL
>            ↑ 底层链表（包含所有元素）
>
> 查找 20 的过程：
>   Level 3: head → 35 (35 > 20, 下降)
>   Level 2: head → 12 → 35 (35 > 20, 停在12, 下降)
>   Level 1: 12 → 20 (找到！)
>   总步数: 4 步  vs  底层链表需要 5 步
> ```
>
> **为什么 Redis 选跳表而不是 B+ 树（用于 SortedSet）？**
>
> | 对比维度 | 跳表（SkipList） | B+ 树 |
> |---------|----------------|-------|
> | 实现复杂度 | 低（几百行代码） | 高（旋转、分裂、合并） |
> | 内存占用 | 中（指针略多） | 低（节点紧凑） |
> | 范围查询 | O(log N) 定位 + O(K) 遍历 | O(log N) 定位 + O(K) 遍历 |
> | 并发支持 | 天然支持局部加锁（只锁影响层） | 需要复杂的 B-link 树或页锁 |
> | 磁盘友好 | 差（随机指针，缓存不友好） | 好（页对齐，适合磁盘 IO） |
>
> Redis 是**内存数据库**，不需要磁盘友好，而且跳表实现简单（对应 antirez 的开发哲学：简单优先）。
> MySQL InnoDB 用 B+ 树是因为数据在磁盘上，B+ 树的叶节点连成链表且每页 16KB 对齐，适合顺序 IO 读取。
>
> 这是一个典型的 **"为所在环境做优化"** 的工程决策案例。
...
> [!note] OS 补充 —— Jedis vs Lettuce 的线程模型对比
>
> ```
> Jedis 线程模型（BIO 阻塞式）:
>
...
> 线程3 ─── Socket3 ─── Redis
>
> 每个线程独占一个 Socket（TCP 连接），1000 个线程 = 1000 个连接
> 优点：简单直观  缺点：线程开销大，连接数受限
>
> Lettuce 线程模型（NIO 非阻塞式）:
>
> 线程1 ─┐
> 线程2 ─┤                  ┌─── Redis
> 线程3 ─┼─── Netty EventLoop ─┤  （单个或少量 TCP 连接）
> ...   ─┘       (epoll)    └─── Redis
>
> 多个线程共用少量连接，通过 Reactor 模式处理请求/响应
> 优点：连接数少、异步高吞吐  缺点：调试相对复杂
> ```
>
> Lettuce 基于 **Netty** + **Reactor 模式**（和 Redis 服务端自身的 epoll 模型异曲同工），
> 一个 Lettuce 连接可以安全地被所有线程共享，底层通过 `Channel.pipeline()` 序列化命令。
>
> 这与 CMU 15-213 讲的 IO 多路复用（`epoll`）是同一思想在客户端侧的应用。
> Jedis 是 **Thread-per-Connection**，Lettuce 是 **IO 多路复用**，两者是完全不同的并发模型。
...
> maxTotal（最大连接数）：
>   = 应用层最大并发请求数
>   如果你的应用有 100 个线程同时使用 Redis，maxTotal 至少设 100
>   但 Redis 有最大连接数限制（默认 10000），不能无限大
>
> maxIdle（最大空闲连接数）：
>   = 常规并发量时保留的"热备"连接数
>   设太小：并发峰值时要临时创建连接（有延迟）
>   设太大：空闲连接占资源（Redis 服务端也要维护连接状态）
>
> maxWait（最大等待时长）：
>   连接池满了，再来请求需要等多久
>   设太小：频繁报 "Could not get a resource from pool" 异常
>   设太大：请求堆积，用户体验差
>
> minIdle（最小空闲连接数）：
>   连接池"预热"的连接数，空闲时也保留这么多连接
>   避免冷启动时的建立连接延迟
> ```
>
> **经验参考值**（中小型服务）：
> ```yaml
> max-active: 8    # = CPU核心数 × 2，满足大部分场景
> max-idle: 8      # = max-active，避免频繁销毁和重建
> min-idle: 2      # 保留少量热备连接
> max-wait: 200ms  # 200ms 超时，快速失败比长时间等待体验更好
> ```
>
> TCP 连接建立需要 **3次握手**（每次约 1~5ms 网络 RTT），连接池正是为了复用这些已建立的连接，节省每次握手的开销。这与 HTTP/1.1 的 `Keep-Alive` 长连接、HTTP/2 的多路复用是同一思想。
