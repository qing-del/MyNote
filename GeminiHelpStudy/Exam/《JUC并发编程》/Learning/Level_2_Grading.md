...
### 极客挑战 1 · `LongAdder` 为什么比 `AtomicLong` 快？（11 / 15）

✅ **答对的部分**：
- 低并发下两者行为相同（CAS base），高并发下 `LongAdder` 触发 `Cell[]` 扩展——方向完全正确。
- `Cell[]` 最大 size 等于 CPU 核心数（初始为 2），正确。
- `@Contended` 的作用：防止多个 `Cell` 落入同一 64 字节缓存行，禁止伪共享——原因讲清楚了。
- `sum()` 不精确、适合软限制场景（如流量统计）而非硬限制——正确，用了准确的对比例子。
...
正确名称是 **MESI 协议**（Modified / Exclusive / Shared / Invalid 四种缓存行状态的首字母缩写）。这不只是笔误的问题——MESI 的四个状态是理解缓存一致性的核心，下面极客挑战 2 的答案里你也用到了，所以这里要明确订正。

**② `sum()` 不精确的方向描述有误（-2）**

> "在高并发下会比精确值略低"

这个方向不对。`LongAdder.sum()` 的实现是 `base + Σ(cells[i])`，它在求和时没有任何锁保护。：
- 如果有线程在 `sum()` 执行期间还在累加（修改 base 或某个 cell），这些新增量**可能被读到，也可能读不到**。所以实际结果可能**偏高**（读到了后续的写），也可能**偏低**（漏掉了并发的写），不是固定方向的偏差，而是"某一时刻的近似快照"。

修正：`sum()` 的返回值是**近似值**，在并发写入期间既可能偏高也可能偏低，不适合需要强一致精确计数的场景。

**③ 缺少对 `@Contended` 填充机制的说明（-1）**

你说了"用了才能防止行级伪共享"，但没有讲清楚 `@Contended` 的实现原理：它通过在 `Cell` 对象前后各填充 **128 字节的 padding**（实际上是冗余字节），使得每个 `Cell` 独占至少一条完整的缓存行，从而物理上隔离不同 `Cell` 的写操作，避免相互失效。

**标准补充**：

```
不加 @Contended:
|-- Cell[0] --| Cell[1] | Cell[2] | Cell[3] --|  ← 同一缓存行(64字节)
  core-1 写      core-2 写  → 互相触发 MESI Invalid，形成伪共享

加了 @Contended:
|==padding==|-- Cell[0] --|==padding==|   ← 独占一整行
|==padding==|-- Cell[1] --|==padding==|   ← 独占一整行
  core-1 和 core-2 各自写自己的行，不互相干扰
```

> **极客挑战 1 小计：11 / 15**
> 核心逻辑抓对了，细节上 MESI 名称、`sum()` 偏差方向、`@Contended` 原理三处需要订正。
