...
- [x] 前端发送json格式请求的时候，接口要接收这个json格式的请求，记得加上`@RequestBody`来解析这个json格式的数据
- [ ] 
...
24. 现在打算加入任务编辑功能，以及后续的管理端。修改的时候发现`task`数据库表中的`coinReward`是驼峰命名法，但是sql是大小写不敏感的，故改为`coin_reward`；管理端做到一半发现似乎没那么简单，先没有继续做了，后续再完成。
25. 后续找导师提交任务2和3之后，导师对我的前端页面进行了修改，其中出现了一个问题，点击`购买`之后就是前端网络包中出现`服务器错误：Content-Type 'application/x-www-form-urlencoded;charset=UTF-8'is not supported`这个内容，这个是前后端交互协议不匹配导致的（PS也有可能是后端接口中的形参不小心加错了注释），可以通过改为发送json格式来尝试解决，后续又出现了`服务器错误：JSON parse error: Cannot deserialize value of type `java.lang.Long`from Object value(token `JsonToken.START_OBJECT`)`，其实就是只传了一个json格式，但是实际上形参是两个`Long`类型，实际上是一个很幽默的错误，其实已经做了修改，但是忘记重启后端了，所以后端没有及时更新上，后续测试已通过。
26. 后续学习了一点Spring的基础，并且学习了自定义starter。
27. 后续打算加入一些处理用户增删改查的逻辑。中途出现了`UserMapper`中一直返回错误null的情况，并且返回值改为`Integer`依旧无法解决，原因是因为不小心写成了`@Select("update ...")`；后续出现了调用阿里云OSS的改造函数删除文件时，因为没有全路径而无法删除，全路径是需要知道修改的日期年份和月份，因为存储路径是`yyyy/MM/fileName.jpg`，所以对数据库进行了修改。
28. 利用SpringAOP加入操作日志记录。用于记录用户对任务增删改查的操作进行记录。在记录日志的时候，保存行参的时候，记得过滤敏感信息，如`password`，也要对`大对象`进行（如头像）等进行压缩处理；刚写好开始运行的时候写了通过类反射获取注解类，但是实际上是添加在方法上的，所以导致`null`异常，后续改为先获取方法再通过反射获取注解类就解决了；
29. 学习maven高级内容：分模块开发、
...
24. 增加编辑功能 + 完善管理者接口和逻辑
	...
	4. [x] 在当前进度测试成功之后上传一次GitHub作于版本管理
	5. [x] 由于后端中安全系统是获取用户名之后再通过用户名去获取玩家信息的
		1. [x] 后端中在`User`表中加入`role`作为权限级别的代表
		2. [x] 给实体类`User`加入`role`这一个成员变量
	6. [x] 数据处理
		1. [x] 给`UserMapper`修改一些语句
		2. [ ] 管理玩家属性的Mapper
		3. [ ] 管理用户的Mapper
		4. [ ] 管理商店的Mapper
	7. [ ] 逻辑处理
	...
	9. [ ] 前端页面开发
		1. [ ] 管理员登录面板
		2. [ ] 
	10. [ ] 完成前后端联桥测试 and 上传GitHub作为一个版本
25. [x] 完成bug的调整
	1. [x] 在后端中加入`BuyRequest`
	2. [x] 修改后端中的`ShopController`
	3. [x] 修改前端中的`Shop.vue`
	4. [x] 前后联桥测试
	5. [x] 上传GitHub做一次版本管理
26. [x] 通过视频课程学习一些基础的Spring原理 ＋ 完成自定义starter
27. [ ] 新增用户增删改查功能
	1. [x] 新增用户`UserController`
	2. [x] 新增`UserService`
	3. [x] 完善上述的增删改查接口与逻辑
		1. [x] 将Auth这个功能拆开做成`Controller`+`Service`+`Mapper`
	4. [x] 加入`UserException` 并 修改全局异常处理器
	5. [x] 加入`UserRequest`实体类
	6. [x] 加入修改用户名的接口 ＋ 完善逻辑
	7. [x] 加入修改密码的接口 + 完善逻辑
	8. [x] 通过 API 测试工具进行测试没有问题
	9. [x] 加入`head_photo`来存放可以访问`url`链接 + 修改实体类
	10. [x] 在`UserMapper`中加入修改`url`的函数
	11. [x] 利用写好的自定义 starter 加入头像上传的接口
	12. [ ] 加入头像获取的接口
	13. [x] 加入头像删除的（接口） -> （逻辑）
		1. [x] 由于在url中已经包含了文件的名字，通过截取获取即可
		2. [x] 在自定义starter中改造删除文件的接口
		3. [ ] 在任务项目中加入修改头像的接口（舍弃）
		4. [x] 实现其接口的逻辑 -> 改造原来上传头像的逻辑
		5. [x] 需要加入一个记录头像上传日期的列`head_photo_update_time`
		6. [x] 修改实体类
		7. [x] 修改Mapper中的sql语句
		8. [x] 完善逻辑
	14. [ ] 加入头像修改的接口
	15. [x] 利用接口测试应用来进行一下测试
	16. [ ] 对新增的接口进行前端的开发
	17. [ ] 进行全后端联桥测试
	18. [ ] 保存到GitHub
28. [ ] 新增记录操作任务日志的功能
	1. [x] 新增`operation_log`数据库表
	2. [x] 新增`OperationLog`实体类
	3. [x] 新增`OpeationLogMapper`用于操作数据
	4. [x] 编写`@Log`注解类
	5. [x] 编写`OperationLogAspect`切面类
	6. [x] 利用SpringAOP完善获取`用户id`的功能
		1. [x] 重新复习一次`ThreadLocal`
		2. [x] 在`JwtFilter`中加入`ThreadLocal`的逻辑
		3. [x] 改造`JwtUitl`工具类中的`generateToken()`方法 -- 加入存放`UserId`
		4. [x] 在切面类中利用`ThreadLocal`来获取`UserId`
	7. [x] 完善切面类的完整逻辑
		1. [x] 捕获异常信息
		2. [x] 保存异常信息
	8. [x] 进行前后端测试
